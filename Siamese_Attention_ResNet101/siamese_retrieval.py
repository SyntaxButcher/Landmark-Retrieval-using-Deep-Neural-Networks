...
Original file is located at
    https://colab.research.google.com/drive/1k9lvM6_-bikPHjjAxY5psy-tDtEH8HgJ
"""
...
def Siamese_bin(input_shape, weights):
  ...
  # Define the L1 distance layer
  L1_layer = tf.keras.layers.Lambda(lambda x: K.abs(x[0] - x[1]))
  ...
  # Compile the Siamese model
  siamese_model.compile(loss=tf.keras.losses.BinaryCrossentropy(), optimizer='adam', metrics=['accuracy'])
...
def compute_distances(model, query_image, query_idx, index_images, ground_truth, images_dir):
    distances = []
    bbox = [int(round(b)) for b in ground_truth[query_idx]['bbx']]
    query_img = process_image(query_image, bbox)

    for j, index_image in tqdm(enumerate(index_images), total=len(index_images), desc="Computing distances"):
        index_img = process_image(index_image)
        distance = model.predict([np.expand_dims(query_img, axis=0), np.expand_dims(index_img, axis=0)])
        distances.append((index_image, distance))
    return distances

def compute_embeddings(model, image_paths, query_idx=None, ground_truth=None):
    if ground_truth is not None:
      images = [process_image(path, bbox =[int(round(b)) for b in ground_truth[query_idx]['bbx']])]
    else:
...
flags.DEFINE_string(
    'weights_file', 'siamese_weights_101_valcon2.h5',
...
def main(argv):
  ...
  query_img = query_list_new[query_idx]

  distances = compute_distances(siamese_model, query_img, query_idx, index_list_new, ground_truth, FLAGS.images_dir)

  sorted_distances = sorted(distances, key=lambda x: x[1])

  with open("sorted_distances.pkl", "wb") as f:
    pickle.dump(sorted_distances, f)

  print(f"Query Image: {query_img}")
  print(f"Similar images: {sorted_distances[0:10][0]}")
###################
